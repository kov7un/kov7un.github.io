...
(function($) {
        ...
        if ($product.length) {
            if (purchasable === 'yes') {
                ...
                if (image_src !== undefined && image_src !== '') {
                    ...
                    $product.find('.woobt-thumb-new').html('<img src="' + image_src + '"/>').show();
                }
                ...
                if (image_src !== undefined && image_src !== '') {
                    ...
                    if ($img.length) {
                        if ($img.attr('data-src') === undefined) {
                            $img.attr('data-src', $img.attr('src')).attr('data-srcset', $img.attr('srcset')).attr('data-sizes', $img.attr('sizes')).removeAttr('srcset').removeAttr('sizes');
                        }
                ...
                if ($img.length) {
                    if ($img.attr('data-src') !== undefined) {
                        ...
                        if ($img.attr('data-sizes') !== undefined) {
                            $img.attr('sizes', $img.attr('data-sizes')).removeAttr('data-sizes');
                        }
                    }
                }
            }
            ...
            if (id > 0) {
                ...
                if (image_src !== undefined && image_src !== '') {
                    ...
                    $products.find('.woobt-product-this .woobt-thumb-new').html('<img src="' + image_src + '"/>').show();
                }
                ...
                $products.attr('data-product-weight', $products.attr('data-product-o_weight'));
                $products.attr('data-product-dimensions', $products.attr('data-product-o_dimensions'));
            }
        }
        woobt_init($wrap);
    });
    $(document).on('found_variation', function(e, t) {
        ...
        if ($product.length) {
            ...
            if (t['image']['url'] && t['image']['srcset']) {
                $product.find('.woobt-thumb-ori').hide();
                $product.find('.woobt-thumb-new').html('<img src="' + t['image']['url'] + '" srcset="' +
                    t['image']['srcset'] + '"/>').show();
            }
            $('.product_meta .sku').html($products.attr('data-product-sku'));
            $('.product_weight, .woocommerce-product-attributes-item--weight .woocommerce-product-attributes-item__value').html($products.attr('data-product-weight'));
            ...
            if (t['image']['url']) {
                ...
                if ($img.length) {
                    if ($img.attr('data-src') === undefined) {
                        $img.attr('data-src', $img.attr('src')).attr('data-srcset', $img.attr('srcset')).attr('data-sizes', $img.attr('sizes'));
                    }
            ...
            if ($products.find('.woobt-product-this').length) {
                ...
                if (t['image']['url'] && t['image']['srcset']) {
                    $products.find('.woobt-product-this .woobt-thumb-ori').hide();
                    $products.find('.woobt-product-this .woobt-thumb-new').html('<img src="' + t['image']['url'] + '" srcset="' +
                        t['image']['srcset'] + '"/>').show();
                }
            }
        }
    ...
    $(document).on('click touch', '.woobt-quantity-input-plus, .woobt-quantity-input-minus', function() {
        var $qty = $(this).closest('.woobt-quantity-input').find('.woobt-qty'),
...
function woobt_check_position($wrap) {
    ...
    if ((woobt_vars.position === 'before') && (woobt_vars.add_to_cart_button === 'main') && ($products.attr('data-product-type') === 'variable') && ($products.attr('data-variables') === 'no' || woobt_vars.variation_selector === 'wpc_radio' || woobt_vars.variation_selector === 'woovr')) {
...
function woobt_calc_price($wrap) {
    ...
    if ($product_this.length) {
        ...
        if (_qty > 0 && _id > 0) {
            ...
            $product_this.find('.woobt-price-new').html(woobt_price_html(_total_ori_regular, _total) + _price_suffix).show();
        } else {
    ...
    if (count > 0) {
        ...
        $additional.html(woobt_vars.additional_price_text + ' ' + woobt_format_price(total) +
    ...
    if ((woobt_vars.change_price !== 'no') && (woobt_vars.add_to_cart_button !== 'separate')) {
...
function woobt_format_price(total) {
    ...
    switch (woobt_vars.price_format) {
        ...
        case '%1$s %2$s':
            ...
            break;
        case '%2$s%1$s':
            total_html += total_formatted + '<span class="woocommerce-Price-currencySymbol">' +
